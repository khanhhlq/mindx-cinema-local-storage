...
(function() {
  ...
  var promises = routeKeys.map(function(item) {
    return fetch('https://api.themoviedb.org/3' + HomeAPIRoutes[item].url + '?api_key=' + TMDB_API_KEY)
      .then(function(response) { return response.json(); });
  });

  Promise.all(promises)
    .then(function(responses) {
      var data = responses.reduce(function(final, current, index) {
        final[routeKeys[index]] = current.results;
        return final;
      }, {});
      ...
      routeKeys.forEach(function(key, index) {
        var moviesHTML = data[key].map(function(item) {
